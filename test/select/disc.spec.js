...
test('itemsRaw()', (t) => {
  ...
  t.equal(res['DL1001-09'], store.getState().graph2.OrderTrackItem['DL1001-09'])
  // OPTION 3 - LOGIN
  store.dispatch(login({ id: 'user1' }))
  const res2 = itemsRaw(store.getState())
  t.equal(size(res2), 2, 'size')
  t.equal(res2['DL1001-09'], store.getState().graph2.OrderTrackItem['DL1001-09'])
  t.equal(res2['DL1001-11'], store.getState().graph2.OrderTrackItem['DL1001-11'])
  // OPTION 2 - DISC ONLY
  store.dispatch(onClick())
  const res3 = itemsRaw(store.getState())
  t.equal(size(res3), 1, 'size')
  t.equal(res3['DL1001-11'], store.getState().graph2.OrderTrackItem['DL1001-11'])
  // console.log(res3)
  t.end()
})
